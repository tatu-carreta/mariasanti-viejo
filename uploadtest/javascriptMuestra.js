$(document).ready(function() {
	...
	$('#upload-button .upload').click(function() {
		...
		$.post(PATH_UPLOAD+'crearMuestra.php', {title: titulo, descript: descripcion, lang:languaje }, function(data){
			var idMuestra = data;
					...
					$.post(PATH_UPLOAD+'uploadMuestra.php', { arrayImage: dataArray[index], muestra: idMuestra}, function(data) {
						...
						if(totalPercent*(x) == 100) {
							...
							$('#loading-content').html('Las fotos han sido publicadas!');
	...
	function deleteSpecific(idDelete){
		...
		if(confirm("Está a punto de eliminar esta imagen. ¿Desea continuar?"))
		{
			...
			$.each(dataArray, function(index, file){
				...
				$('#ulCarga').append('<li class="imageLi" data="'+dataArray[index].id+'"><table><tr><td><img class="imagenCargada" src="'+dataArray[index].imagen+'"/> </td></tr></table><a class="delete" onclick="deleteSpecific('+dataArray[index].id+'); return false">Eliminar</a><a class="abrir" onclick="displayText($(this))">Abrir</a><div class="divHover"></div></li>'); 		
			});
