$(document).ready(function() {
	...
	$('#drop-files').bind('drop', function(e) {
		...
		$.each(files, function(index, file) {
			...
			if(counter >= maxFiles)
			{
				alert("Ha alcanzado el máximo de imágenes");
				return false;
			}
	...
	$('#upload-button .upload').click(function() {
		...
		var titulo = $("#titulo").val();
				...
				$.post(PATH_UPLOAD+'uploadSlide.php', { arrayImage: dataArray[index], order: orden}, function(data) {
					...
					//window.location.reload();
							...
							alert('El slide ha sido modificado correctamente.');
	...
	if(window.localStorage.length > 0) {
            ...
            if(confirm("Está a punto de eliminar esta imagen. ¿Desea continuar?"))
		{
			...
			// Just some grammatical adjustments
                    ...
                    $.post(PATH_UPLOAD+'borrarImagenSlide.php', {image: idDelete }, function(data){
                        //alert(data);
						counter = (dataArray.length) + (dataArray2.length);
                    });
		}
		else
		{
